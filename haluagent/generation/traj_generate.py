...
with open("prompt/trajectory_generation.jsonl", "r") as f:
    global prompt
    prompt = []
    for line in f:
...
def generate_p(file, output):
    ...
    try:
        for d in data:
            ...
            prompt = prompt[:26]
            # print(len(prompt))
            query = {"role": "user", "content": "问题：" + d["question"] + " 回复：" + d["answer"]}
            ...
            try:
                while True:
                    ...
                    if "split" in action:
                        ...
                        response = {"role": "user", "content": response+"这段文本的字数是"+str(count)}
                        label_list.append(label)
